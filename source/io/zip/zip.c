...
typedef struct linkedlist_datablock_internal_s
{
  ...
  uLong  avail__inhis_block;
  uLong  filled__inhis_block;
...
local linkedlist_datablock_internal* allocate_new_datablock()
{
    ...
    if (ldi!=NULL)
    {
        ...
        ldi->filled__inhis_block = 0 ;
        ldi->avail__inhis_block = SIZEDATA_INDATABLOCK ;
    }
    return ldi;
}
...
local int add_data_in_datablock(linkedlist_data* ll, const void* buf, uLong len)
{
    ...
    while (len>0)
    {
        ...
        if (ldi->avail__inhis_block < len)
            copy_this = (uInt)ldi->avail__inhis_block;
        else
            copy_this = (uInt)len;

        to_copy = &(ldi->data[ldi->filled__inhis_block]);

        for (i=0;i<copy_this;i++)
            *(to_copy+i)=*(from_copy+i);

        ldi->filled__inhis_block += copy_this;
        ldi->avail__inhis_block -= copy_this;
        from_copy += copy_this ;
        len -= copy_this;
    }
    return ZIP_OK;
...
extern int ZEXPORT zipClose (zipFile file, const char* global_comment)
{
    ...
    if (err==ZIP_OK)
    {
        ...
        while (ldi!=NULL)
        {
            if ((err==ZIP_OK) && (ldi->filled__inhis_block>0))
            {
                if (ZWRITE64(zi->z_filefunc,zi->filestream, ldi->data, ldi->filled__inhis_block) != ldi->filled__inhis_block)
                    err = ZIP_ERRNO;
            }

            size_centraldir += ldi->filled__inhis_block;
            ldi = ldi->next_datablock;
        }
    }
