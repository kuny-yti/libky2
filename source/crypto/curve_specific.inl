#ifndef CURVE_SPECIFIC_H
#define CURVE_SPECIFIC_H
...
#endif /* CryptoWordSize */

#if CryptoSupportSecp160R1 || CryptoSupportSecp192R1 || \
    CryptoSupportSecp224R1 || CryptoSupportSecp256R1
static void double_jacobian_default(word_t * X1, word_t * Y1,
                                    word_t * Z1, ecc_curve* curve)
{
    /* t1 = X, t2 = Y, t3 = Z */
    word_t t4[CryptoMaxWord];
    word_t t5[CryptoMaxWord];
    int8_t num_words = curve->num_words;

    if (vli_isZero(Z1, num_words)) {
        return;
    }
    ...
    vli_modAdd(X1, X1, Z1, curve->p, num_words); /* t1 = x1 + z1^2 */
    vli_modAdd(Z1, Z1, Z1, curve->p, num_words); /* t3 = 2*z1^2 */
    vli_modSub(Z1, X1, Z1, curve->p, num_words); /* t3 = x1 - z1^2 */
    vli_modMult_fast(X1, X1, Z1, curve);                /* t1 = x1^2 - z1^4 */

    vli_modAdd(Z1, X1, X1, curve->p, num_words); /* t3 = 2*(x1^2 - z1^4) */
    vli_modAdd(X1, X1, Z1, curve->p, num_words); /* t1 = 3*(x1^2 - z1^4) */
    if (vli_testBit(X1, 0))
    {
        word_t l_carry = vli_add(X1, X1, curve->p, num_words);
        vli_rshift1(X1, num_words);
        X1[num_words - 1] |= l_carry << (WORD_BITS - 1);
    }
    else
        vli_rshift1(X1, num_words);
    ...
    vli_modSub(Z1, Z1, t5, curve->p, num_words); /* t3 = B^2 - A */
    vli_modSub(Z1, Z1, t5, curve->p, num_words); /* t3 = B^2 - 2A = x3 */
    vli_modSub(t5, t5, Z1, curve->p, num_words); /* t5 = A - x3 */
    vli_modMult_fast(X1, X1, t5, curve);         /* t1 = B * (A - x3) */
    vli_modSub(t4, X1, t4, curve->p, num_words); /* t4 = B * (A - x3) - y1^4 = y3 */

    vli_set(X1, Z1, num_words);
    vli_set(Z1, Y1, num_words);
    vli_set(Y1, t4, num_words);
}

/* Computes result = x^3 + ax + b. result must not overlap x. */
static void x_side_default(word_t *result, const word_t *x, ecc_curve* curve)
{
    word_t _3[CryptoMaxWord] = {3}; /* -a = 3 */
    int8_t num_words = curve->num_words;

    vli_modSquare_fast(result, x, curve);                      /* r = x^2 */
    vli_modSub(result, result, _3, curve->p, num_words);       /* r = x^2 - 3 */
    vli_modMult_fast(result, result, x, curve);                /* r = x^3 - 3x */
    vli_modAdd(result, result, curve->b, curve->p, num_words); /* r = x^3 - 3x + b */
}
#endif

#if CryptoSupportCompressed
#if CryptoSupportSecp160R1 || CryptoSupportSecp192R1 || \
    CryptoSupportSecp256R1 || CryptoSupportSecp256K1
/* Compute a = sqrt(a) (mod curve_p). */
static void mod_sqrt_default(word_t *a, ecc_curve* curve)
{
    int16_t i;
    word_t p1[CryptoMaxWord] = {1};
    word_t l_result[CryptoMaxWord] = {1};
    int8_t num_words = curve->num_words;

    /* When curve->p == 3 (mod 4), we can compute
       sqrt(a) = a^((curve->p + 1) / 4) (mod curve->p). */
    vli_add(p1, curve->p, p1, num_words); /* p1 = curve_p + 1 */
    for (i = vli_numBits(p1, num_words) - 1; i > 1; --i)
    {
        vli_modSquare_fast(l_result, l_result, curve);
        if (vli_testBit(p1, i))
            vli_modMult_fast(l_result, l_result, a, curve);
    }
    vli_set(a, l_result, num_words);
}
#endif
#endif
...
static void vli_mmod_fast_secp160r1(word_t *result, word_t *product);
#endif

static struct ecc_curve curve_secp160r1 =
{
    num_words_secp160r1,
    num_bytes_secp160r1,
    161, /* num_n_bits */
...
ecc_curve* ecc_secp160r1(void) { return &curve_secp160r1;}

#if (CryptoOptimizatLevel > CO_LevelSystem && !asm_mmod_fast_secp160r1)
/* Computes result = product % curve_p
    see http://www.isys.uni-klu.ac.at/PDF/2001-0126-MT.pdf page 354

    Note that this only works if log2(omega) < log2(p) / 2 */
static void omega_mult_secp160r1(word_t *result, const word_t *right);
#if CryptoWordSize == 8
static void vli_mmod_fast_secp160r1(word_t *result, word_t *product)
{
    word_t tmp[2 * num_words_secp160r1];
    word_t copy;
    ...
    while (vli_cmp_unsafe(result, curve_secp160r1.p, num_words_secp160r1) > 0) {
...
static void vli_mmod_fast_secp160r1(word_t *result, word_t *product)
{
    ...
    if (vli_cmp_unsafe(result, curve_secp160r1.p, num_words_secp160r1) > 0) {
        vli_sub(result, result, curve_secp160r1.p, num_words_secp160r1);
    }
}
#endif
...
#if (CryptoOptimizatLevel > CO_LevelSystem)
static void vli_mmod_fast_secp192r1(word_t *result, word_t *product);
#endif

static struct ecc_curve curve_secp192r1 =
{
    num_words_secp192r1,
    num_bytes_secp192r1,
    192, /* num_n_bits */
...
ecc_curve* ecc_secp192r1(void) { return &curve_secp192r1; }
...
static void vli_mmod_fast_secp192r1(uint8_t *result, uint8_t *product)
{
    ...
    while (carry || vli_cmp_unsafe(curve_secp192r1.p, result, num_words_secp192r1) != 1)
...
static void vli_mmod_fast_secp192r1(uint64_t *result, uint64_t *product)
{
    ...
    carry = (int)vli_add(result, result, tmp, num_words_secp192r1);
...
#if CryptoSupportCompressed
static void mod_sqrt_secp224r1(word_t *a, ecc_curve* curve);
#endif
#if (CryptoOptimizatLevel > CO_LevelSystem)
static void vli_mmod_fast_secp224r1(word_t *result, word_t *product);
#endif

static struct ecc_curve curve_secp224r1 =
{
    num_words_secp224r1,
    num_bytes_secp224r1,
    224, /* num_n_bits */
...
ecc_curve* ecc_secp224r1(void) { return &curve_secp224r1; }


#if CryptoSupportCompressed
/* Routine 3.2.4 RS;  from http://www.nsa.gov/ia/_files/nist-routines.pdf */
static void mod_sqrt_secp224r1_rs(word_t *d1,
                                  word_t *e1,
                                  word_t *f1,
                                  const word_t *d0,
                                  const word_t *e0,
...
static void mod_sqrt_secp224r1_rss(word_t *d1,
                                   word_t *e1,
                                   word_t *f1,
                                   const word_t *d0,
                                   const word_t *e0,
...
static void mod_sqrt_secp224r1_rm(word_t *d2,
                                  word_t *e2,
                                  word_t *f2,
                                  const word_t *c,
                                  const word_t *d0,
                                  ...
                                  const word_t *e1)
{
    word_t t1[num_words_secp224r1];
    word_t t2[num_words_secp224r1];
...
static void mod_sqrt_secp224r1_rp(word_t *d1,
                                  word_t *e1,
                                  word_t *f1,
                                  const word_t *c,
                                  const word_t *r)
{
    int8_t i;
    int8_t pow2i = 1;
    word_t d0[num_words_secp224r1];
    word_t e0[num_words_secp224r1] = {1}; /* e0 <-- 1 */
    word_t f0[num_words_secp224r1];
...
{
    int16_t i;
    word_t e1[num_words_secp224r1];
    word_t f1[num_words_secp224r1];
    word_t d0[num_words_secp224r1];
    word_t e0[num_words_secp224r1];
    word_t f0[num_words_secp224r1];
    word_t d1[num_words_secp224r1];
...
static void vli_mmod_fast_secp224r1(uint8_t *result, uint8_t *product)
{
    ...
    if (carry < 0) {
        ...
        while (carry || vli_cmp_unsafe(curve_secp224r1.p, result, num_words_secp224r1) != 1) {
...
static void vli_mmod_fast_secp224r1(uint64_t *result, uint64_t *product)
{
    ...
    if (carry < 0) {
        ...
        while (vli_cmp_unsafe(curve_secp224r1.p, result, num_words_secp224r1) != 1) {
...
#if (CryptoOptimizatLevel > CO_LevelSystem)
static void vli_mmod_fast_secp256r1(word_t *result, word_t *product);
#endif

static struct ecc_curve curve_secp256r1 = {
    num_words_secp256r1,
    num_bytes_secp256r1,
    256, /* num_n_bits */
    { BYTES_TO_WORDS_8(FF, FF, FF, FF, FF, FF, FF, FF),
...
ecc_curve* ecc_secp256r1(void) { return &curve_secp256r1; }


#if (CryptoOptimizatLevel > CO_LevelSystem && !asm_mmod_fast_secp256r1)
/* Computes result = product % curve_p
   from http://www.nsa.gov/ia/_files/nist-routines.pdf */
#if CryptoWordSize == 1
static void vli_mmod_fast_secp256r1(uint8_t *result, uint8_t *product) {
    ...
    if (carry < 0) {
        ...
        while (carry || vli_cmp_unsafe(curve_secp256r1.p, result, num_words_secp256r1) != 1) {
...
static void vli_mmod_fast_secp256r1(uint64_t *result, uint64_t *product) {
    ...
    tmp[2] = product[6];
    tmp[3] = product[7];
    carry = (int)vli_add(tmp, tmp, tmp, num_words_secp256r1);
    carry += vli_add(result, result, tmp, num_words_secp256r1);
...
static void double_jacobian_secp256k1(word_t * X1,
                                      word_t * Y1,
                                      word_t * Z1,
                                      ecc_curve* curve);
static void x_side_secp256k1(word_t *result, const word_t *x, ecc_curve* curve);
#if (CryptoOptimizatLevel > CO_LevelSystem)
static void vli_mmod_fast_secp256k1(word_t *result, word_t *product);
#endif

static struct ecc_curve curve_secp256k1 = {
    num_words_secp256k1,
    num_bytes_secp256k1,
    256, /* num_n_bits */
...
ecc_curve* ecc_secp256k1(void) { return &curve_secp256k1; }
...
static void double_jacobian_secp256k1(word_t * X1,
                                      ...
                                      ecc_curve* curve) {
    /* t1 = X, t2 = Y, t3 = Z */
    word_t t4[num_words_secp256k1];
    word_t t5[num_words_secp256k1];
    ...
    if (vli_testBit(Y1, 0)) {
        word_t carry = vli_add(Y1, Y1, curve->p, num_words_secp256k1);
...
#if (CryptoOptimizatLevel > CO_LevelSystem && !asm_mmod_fast_secp256k1)
static void omega_mult_secp256k1(word_t *result, const word_t *right);
static void vli_mmod_fast_secp256k1(word_t *result, word_t *product) {
    word_t tmp[2 * num_words_secp256k1];
    word_t carry;
    ...
    if (vli_cmp_unsafe(result, curve_secp256k1.p, num_words_secp256k1) > 0) {
        vli_sub(result, result, curve_secp256k1.p, num_words_secp256k1);
    }
}
...
static void omega_mult_secp256k1(uint8_t * result, const uint8_t * right) {
    ...
    vli_muladd(0xD1, right[0], &r0, &r1, &r2);
    result[0] = r0;
    r0 = r1;
    r1 = r2;
    /* r2 is still 0 */
    ...
    for (k = 1; k < num_words_secp256k1; ++k) {
        vli_muladd(0x03, right[k - 1], &r0, &r1, &r2);
        vli_muladd(0xD1, right[k], &r0, &r1, &r2);
        result[k] = r0;
        r0 = r1;
        r1 = r2;
        r2 = 0;
    }
    vli_muladd(0x03, right[num_words_secp256k1 - 1], &r0, &r1, &r2);
    result[num_words_secp256k1] = r0;
    result[num_words_secp256k1 + 1] = r1;
    /* add the 2^32 multiple */
    result[4 + num_words_secp256k1] =
        vli_add(result + 4, result + 4, right, num_words_secp256k1);
}
#elif CryptoWordSize == 4
static void omega_mult_secp256k1(uint32_t * result, const uint32_t * right) {
    ...
    result[1 + num_words_secp256k1] =
        vli_add(result + 1, result + 1, right, num_words_secp256k1);
}
#else
static void omega_mult_secp256k1(uint64_t * result, const uint64_t * right) {
    ...
    /* Multiply by (2^32 + 2^9 + 2^8 + 2^7 + 2^6 + 2^4 + 1). */
    for (k = 0; k < num_words_secp256k1; ++k) {
        vli_muladd(0x1000003D1ull, right[k], &r0, &r1, &r2);
        result[k] = r0;
        r0 = r1;
        r1 = r2;
        r2 = 0;
